...
// Restore the URIBitcoin to it's stringy glory.
URIBitcoin.prototype.toString = function () {
    var str = "";
    if (this.scheme) {
        str += this.scheme + ":";
    }
...
(function () {
    ...
    URIBitcoin.prototype.resolve = function (base) {
        var target = new URIBitcoin();
        if (this.scheme) {
    ...
    var query_decode = function (txt) {
        return decodeURI(txt.replace(/\+/, ' '));
    };

    URIBitcoin.prototype.query_form = function () {
        if (!this.query) {
            return {};
        }
        var kvp = this.query.split(/[&;]/);
        var rv = ({});
        for (var i = 0; i < kvp.length; ++i) {
            var kva = kvp[i].split(/\=/);
            var key = query_decode(kva[0]);
            var val = query_decode(kva[1]);
            rv[key] = val;
        }
        return rv;
    };
